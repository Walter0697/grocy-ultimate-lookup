...
def test_auto_created_store_upserts_updates_and_reads_product(tmp_path) -> None:
    ...
    store.upsert(product_id=22, barcode="654321", source="local_confirmed")
...
def test_auto_created_store_delete_missing_product_returns_false(tmp_path) -> None:
    store = AutoCreatedProductStore(str(tmp_path / "auto-created.sqlite3"))

    assert store.delete(22) is False
